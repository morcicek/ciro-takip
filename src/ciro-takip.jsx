...
const SUPABASE_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBrbGtpd29rdGpsamFtd3Jrd3RiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM2ODg2MjksImV4cCI6MjA4OTI2NDYyOX0.cP6mGfAEfE6i_GAm9cW94Wj2RvslP969YLHk8fBalZk';
...
const mergeMonth = (data) => {
  ...
  return {
    ...data,
    sabitMasraflar: SABIT_KATEGORILER.map((k) => ({
      id: k.id,
      ad: k.ad,
      tutar: tutarMap[k.id] || '',
      sabit: true,
...
const upsertDay = async (date, veri) => {
  await supabase
    .from('gunluk_kayitlar')
    .upsert(
      { tarih: date, veri, guncelleme: new Date().toISOString() },
      { onConflict: 'tarih' },
    );
};
...
const upsertMonth = async (y, m, veri) => {
  await supabase
    .from('aylik_kayitlar')
    .upsert(
      { yil: y, ay: m, veri, guncelleme: new Date().toISOString() },
      { onConflict: 'yil,ay' },
    );
};
...
const calcOto = (ciro) => {
  const paketTop =
    num(ciro.trendyol) +
    num(ciro.yemeksepeti) +
    num(ciro.getir) +
    num(ciro.migros);
  return {
    kart: num(ciro.kart) * 0.02,
    paket: paketTop * 0.1,
    yemek: num(ciro.yemek) * 0.1,
    paketTop,
  };
};
...
const calcDay = (d) => {
  const paketTop =
    num(d.ciro.trendyol) +
    num(d.ciro.yemeksepeti) +
    num(d.ciro.getir) +
    num(d.ciro.migros);
  const ciro =
    num(d.ciro.nakit) + num(d.ciro.kart) + paketTop + num(d.ciro.yemek);
  const oto = calcOto(d.ciro);
  const otoTotal = oto.kart + oto.paket + oto.yemek;
  const kapida = num(d.kapidaOdeme) * 0.1;
  ...
  const borek = BOREK_URUNLER.reduce(
    (s, u) => s + num(urunler[u.id]) * u.maliyet,
    0,
  );
...
const calcStats = (dayVeriList, monthVeri) => {
  ...
  dayVeriList.forEach((d, i) => {
    ...
    totalCiro += c.ciro;
    totalMasraf += c.masraf;
    totalBorek += c.borek;
    const ur = d.borek?.urunler || {};
    BOREK_URUNLER.forEach((u) => {
      borekTotals[u.id] += num(ur[u.id]);
    });
    if (c.ciro > 0) activeDays++;
  });

  const sabitTotal = (monthVeri.sabitMasraflar || []).reduce(
    (s, x) => s + num(x.tutar),
    0,
  );
  ...
  return {
    ...
    avgBorek:
      activeDays > 0
        ? Object.values(borekTotals).reduce((a, b) => a + b, 0) / activeDays
        : 0,
    dailyCiro,
    activeDays,
    borekTotals,
...
export default function App() {
  ...
  // Otomatik kaydet (debounce) — loading sırasında kaydetme
  useEffect(() => {
    if (loading) return;
    const t = setTimeout(() => {
      upsertDay(date, dayData);
    }, 1000);
    return () => clearTimeout(t);
  }, [date, dayData]);

  const updateCiro = (f, v) =>
    setDayData((p) => ({ ...p, ciro: { ...p.ciro, [f]: v } }));
  const updateBorek = (f, v) =>
    setDayData((p) => ({
      ...p,
      borek: { urunler: { ...(p.borek?.urunler || {}), [f]: v } },
    }));
  const addEleman = () =>
    setDayData((p) => ({
      ...p,
      gunlukEleman: [
        ...(p.gunlukEleman || []),
        { id: Date.now(), ad: '', tutar: '' },
      ],
    }));
  const updateEleman = (id, f, v) =>
    setDayData((p) => ({
      ...p,
      gunlukEleman: p.gunlukEleman.map((x) =>
        x.id === id ? { ...x, [f]: v } : x,
      ),
    }));
  const delEleman = (id) =>
    setDayData((p) => ({
      ...p,
      gunlukEleman: p.gunlukEleman.filter((x) => x.id !== id),
    }));
  const addMasraf = () =>
    setDayData((p) => ({
      ...p,
  ...
  const updateMasraf = (id, f, v) =>
    setDayData((p) => ({
      ...p,
      masraflar: p.masraflar.map((x) => (x.id === id ? { ...x, [f]: v } : x)),
    }));
  const delMasraf = (id) =>
    setDayData((p) => ({
      ...p,
      masraflar: p.masraflar.filter((x) => x.id !== id),
    }));

  const updateStok = (f, v) =>
    setDayData((p) => ({ ...p, stok: { ...(p.stok || {}), [f]: v } }));
    ...
    >
      ...
      >
        <div style={S.navLeft}>
          ...
          <div>
            ...
            <div style={S.navSub}>
              {view === 'entry' ? 'Daily Logs' : 'Dashboard'}
            </div>
          </div>
        </div>
        <div style={S.navRight}>
          ...
          <button
            ...
            style={{ ...S.navBtn, ...(view === 'entry' ? S.navBtnActive : {}) }}
            title="Günlük Giriş"
          >
            <svg
              width="18"
      ...
      {view === 'entry' ? (
        <EntryView
          ...
          calc={calc}
          loading={loading}
          updateCiro={updateCiro}
          updateBorek={updateBorek}
          addEleman={addEleman}
          updateEleman={updateEleman}
          delEleman={delEleman}
          addMasraf={addMasraf}
          updateMasraf={updateMasraf}
          delMasraf={delMasraf}
          updateStok={updateStok}
          updateKapida={updateKapida}
          onSave={handleSave}
          saving={saving}
          dark={dark}
        />
  ...
  if (loading)
    return (
      <div style={S.loadWrap}>
        <div style={S.spinner} />
        <div style={{ color: '#6b7280', marginTop: 12 }}>Yükleniyor...</div>
      </div>
    );
      ...
      <div style={S.entryInner}>
        ...
        <Section icon="🧾" color="#dc2626" title="Günlük Masraflar" dark={dark}>
              ...
              (dayData.gunlukEleman || []).map((x) => (
                <div key={x.id} style={S.masrafRow}>
                  <input
                    placeholder="Name"
                    value={x.ad}
                    onChange={(e) => updateEleman(x.id, 'ad', e.target.value)}
                  ...
                  <input
                    ...
                    value={x.tutar}
                    onChange={(e) =>
                      updateEleman(x.id, 'tutar', e.target.value)
                    }
                    style={{ ...S.input, flex: 1 }}
                  />
                  <button onClick={() => delEleman(x.id)} style={S.delBtn}>
                    🗑
                  </button>
                </div>
              ))
          ...
          <div style={S.subSection}>
            ...
            <div style={S.masrafRow}>
              ...
              >
                -{fmt(num(dayData.kapidaOdeme) * 0.1)} ₺
              </div>
            </div>
            {num(dayData.kapidaOdeme) > 0 && (
              <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 4 }}>
                {fmt(num(dayData.kapidaOdeme))} ₺ × %10 ={' '}
                {fmt(num(dayData.kapidaOdeme) * 0.1)} ₺ masraf
              </div>
            )}
          </div>
              ...
              (dayData.masraflar || []).map((x) => (
                <div key={x.id} style={S.masrafRow}>
                  <input
                    ...
                    value={x.ad}
                    onChange={(e) => updateMasraf(x.id, 'ad', e.target.value)}
                    style={{ ...S.input, flex: 2 }}
                  />
                  <input
                    placeholder="Amount"
                    ...
                    value={x.tutar}
                    onChange={(e) =>
                      updateMasraf(x.id, 'tutar', e.target.value)
                    }
                    style={{ ...S.input, flex: 1 }}
                  />
                  <button onClick={() => delMasraf(x.id)} style={S.delBtn}>
                    🗑
                  </button>
                </div>
              ))
          ...
          <div style={S.sectionTotal}>
            ...
            <span style={{ ...S.sectionTotalVal, color: '#dc2626' }}>
              ₺{fmt(calc.masraf)}
            </span>
          </div>
        </Section>
        ...
        <Section icon="🥐" color="#d97706" title="Börek Maliyeti" dark={dark}>
            ...
            {BOREK_URUNLER.map((u) => (
              <Field key={u.id} label={u.ad.toUpperCase()}>
                <input
                  ...
                  value={urunler[u.id] || ''}
                  onChange={(e) => updateBorek(u.id, e.target.value)}
                  style={S.input}
                />
                {num(urunler[u.id]) > 0 && (
                  <div style={S.otoHint}>
                    {num(urunler[u.id])} × ₺{u.maliyet.toLocaleString('tr-TR')}{' '}
                    = ₺{fmt(num(urunler[u.id]) * u.maliyet)}
                  </div>
                )}
              </Field>
            ))}
          ...
          <div style={S.sectionTotal}>
            ...
            <span style={{ ...S.sectionTotalVal, color: '#d97706' }}>
              ₺{fmt(calc.borek)}
            </span>
          </div>
        </Section>
        ...
        <Section icon="📦" color="#0891b2" title="Stok Yönetimi" dark={dark}>
          <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>
            Mevcut stok miktarını girin — börek satışları otomatik düşülür
          </div>
          <div style={S.grid2}>
            {BOREK_URUNLER.map((u) => {
              ...
              const pct =
                girilen > 0 ? Math.max(0, (kalan / girilen) * 100) : 0;
              const renk =
                kalan < 0 ? '#dc2626' : pct < 20 ? '#f59e0b' : '#16a34a';
              return (
                <div
                  key={u.id}
                  style={{
        ...
        <Section
          icon="💰"
          color="#16a34a"
          title="Net Kar (Net Profit)"
          dark={dark}
        >
          <div
            style={{
              ...S.profitBox,
              borderColor: calc.net >= 0 ? '#bfdbfe' : '#fecaca',
              background: calc.net >= 0 ? '#eff6ff' : '#fff1f2',
            }}
          >
            ...
            <div
              style={{
                ...S.profitVal,
                color: calc.net >= 0 ? '#1d4ed8' : '#dc2626',
              }}
            >
              ₺{fmt(calc.net)}
            </div>
...
function DashboardView({ y, m, setY, setM, dark }) {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    const [dayList, md] = await Promise.all([
      fetchMonthDays(y, m),
      fetchMonth(y, m),
    ]);
    setMonthData(md);
  ...
  const updSabit = async (id, f, v) => {
    const updated = {
      ...monthData,
      sabitMasraflar: monthData.sabitMasraflar.map((x) =>
        x.id === id ? { ...x, [f]: v } : x,
      ),
    };
  ...
  const addSabit = async () => {
    const updated = {
      ...monthData,
      sabitMasraflar: [
        ...monthData.sabitMasraflar,
        { id: Date.now().toString(), ad: '', tutar: '', sabit: false },
      ],
    };
    setMonthData(updated);
    await upsertMonth(y, m, updated);
  ...
  const delSabit = async (id) => {
    const updated = {
      ...monthData,
      sabitMasraflar: monthData.sabitMasraflar.filter((x) => x.id !== id),
    };
    setMonthData(updated);
    await upsertMonth(y, m, updated);
    const dayList = await fetchMonthDays(y, m);
  ...
  return (
    <div style={{ ...SD.landing, background: dark ? '#0f172a' : '#f3f4f6' }}>
      <div
        style={{ ...SD.leftPanel, background: dark ? '#0f172a' : '#f3f4f6' }}
      >
        <div style={SD.leftInner}>
          <div style={SD.landingHeader}>
            <h1 style={SD.landingTitle}>Aylık Özet</h1>
            <div style={SD.monthNavRow}>
              <button onClick={prevMonth} style={SD.monthBtn}>
                ‹
              </button>
              <span style={SD.monthLabel}>
                {MONTHS[m]} {y}
              </span>
              <button onClick={nextMonth} style={SD.monthBtn}>
                ›
              </button>
            </div>
          </div>
          ...
          <div style={SD.metricsGrid}>
            <MiniMetric
              label="Aylık Ciro"
              value={`₺${fmt(stats.totalCiro)}`}
              color="#2563eb"
              bg="#eff6ff"
            />
            <MiniMetric
              label="Net Kar"
              value={`₺${fmt(stats.netKar)}`}
              color={stats.netKar >= 0 ? '#16a34a' : '#dc2626'}
              bg={stats.netKar >= 0 ? '#f0fdf4' : '#fff1f2'}
              dark={dark}
            />
            <MiniMetric
              label="Ort. Günlük"
              value={`₺${fmt(stats.avgCiro)}`}
              color="#7c3aed"
              bg="#f5f3ff"
              dark={dark}
            />
              ...
              {stats.dailyCiro.map(({ day, ciro }) => (
                ...
                >
                  <div
                    style={{
                      width: '100%',
                      background: ciro > 0 ? '#93c5fd' : '#f3f4f6',
                      borderRadius: '2px 2px 0 0',
                      height: `${(ciro / maxCiro) * 100}%`,
                      minHeight: ciro > 0 ? '3px' : '2px',
                    }}
                  />
                  <div
                    style={{
          ...
          >
                ...
                <div key={x.id} style={SD.sabitRow}>
                  {x.sabit ? (
                    ...
                    <input
                      ...
                      value={x.ad}
                      onChange={(e) => updSabit(x.id, 'ad', e.target.value)}
                      style={{
                        ...SD.sabitInput,
                        width: 90,
                        textAlign: 'left',
                  ...
                  >
                    <div style={SD.sabitInputWrap}>
                      ...
                      <input
                        ...
                        value={x.tutar}
                        onChange={(e) =>
                          updSabit(x.id, 'tutar', e.target.value)
                        }
                        style={SD.sabitInput}
                      />
                    </div>
            ...
            <div style={SD.card}>
              ...
              <div style={{ marginTop: 8 }}>
                <KirilimRow
                  label="Ciro"
                  val={`₺${fmt(stats.totalCiro)}`}
                  color="#2563eb"
                />
                <KirilimRow
                  label="Günlük Masraf"
                  val={`-₺${fmt(stats.totalMasraf)}`}
                  color="#dc2626"
                />
                <KirilimRow
                  label="Börek Maliyet"
                  val={`-₺${fmt(stats.totalBorek)}`}
                  color="#d97706"
                />
                <KirilimRow
                  label="Sabit Masraf"
          ...
          <div style={SD.card}>
            <div style={SD.cardTitle}>
              Börek Performansı — Aylık Toplam Adet
            </div>
            <div style={{ marginTop: 12 }}>
                  ...
                  <div key={u.id} style={{ marginBottom: 12 }}>
                    ...
                    >
                      ...
                      <span style={{ color: colors[i], fontWeight: 700 }}>
                        ...
                        <span style={{ fontWeight: 400, fontSize: 12 }}>
                          {stats.activeDays > 0
                            ? (total / stats.activeDays).toFixed(1)
                            : 0}{' '}
                          ort.
                        </span>
                      </span>
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: `${maxBorek > 0 ? (total / maxBorek) * 100 : 0}%`,
                          background: colors[i],
                          borderRadius: 3,
                        }}
                      />
  ...
  return (
    <div style={SD.card}>
      ...
      <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 12 }}>
        Bugünün stok girişlerine göre kalan
      </div>
      {BOREK_URUNLER.map((u, i) => {
...
function KirilimRow({ label, val, color, bold }) {
  return (
    ...
    >
      <span
        style={{ fontSize: 12, color: '#6b7280', fontWeight: bold ? 600 : 400 }}
      >
        {label}
      </span>
      <span style={{ fontSize: 13, color, fontWeight: bold ? 700 : 500 }}>
        {val}
      </span>
    </div>
  );
