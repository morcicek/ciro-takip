...
const BOREK_URUNLER = [
  { id: "buyuk",   ad: "Büyük Tepsi",        maliyet: 1513, birim: "adet" },
  { id: "kiyma",   ad: "Kıymalı Kol Böreği", maliyet: 336,  birim: "kg"   },
  { id: "kucuk",   ad: "Küçük Tepsi",        maliyet: 672,  birim: "adet" },
  { id: "baklava", ad: "Baklava",            maliyet: 723,  birim: "adet" },
];
...
const defaultFaturalar = () => FATURA_KATEGORILER.map(k => ({ id: k.id, ad: k.ad, tutar: "" }));

const mergeMonth = (data) => {
  if (!data) return { sabitMasraflar: defaultSabitler(), faturalar: defaultFaturalar() };
  ...
  (data.faturalar || []).forEach(x => { if (faturaIds.includes(x.id)) faturaTutarMap[x.id] = x.tutar; });
  return {
    ...data,
    sabitMasraflar: SABIT_KATEGORILER.map(k => ({ id: k.id, ad: k.ad, tutar: tutarMap[k.id] || "", sabit: true })),
    faturalar: FATURA_KATEGORILER.map(k => ({ id: k.id, ad: k.ad, tutar: faturaTutarMap[k.id] || "" })),
  };
};
...
const calcDay = (d) => {
  ...
  const kapida   = num(d.kapidaOdeme) * 0.10;
  const eleman   = (d.gunlukEleman || []).reduce((s, x) => s + num(x.tutar), 0);
  const diger    = (d.masraflar    || []).reduce((s, x) => s + num(x.tutar), 0);
  const masraf   = otoTotal + kapida + eleman + diger;
  const urunler  = d.borek?.urunler || {};
  const sarf     = (d.sarfMalzeme || []).reduce((s, x) => s + num(x.tutar), 0);
  const borek    = BOREK_URUNLER.reduce((s, u) => s + num(urunler[u.id]) * u.maliyet, 0) + sarf;
...
const calcStats = (dayVeriList, monthVeri) => {
  ...
  const faturaTotal = (monthVeri.faturalar || []).reduce((s, x) => s + num(x.tutar), 0);
  const netKar = totalCiro - totalMasraf - totalBorek - sabitTotal - faturaTotal;
  return {
    totalCiro, totalMasraf, totalBorek, sabitTotal, faturaTotal, netKar,
...
export default function App() {
  ...
  // Otomatik kaydet (debounce) — sadece veri yüklendikten sonra kaydet
  useEffect(() => {
    if (!initialized || loading) return;
    const t = setTimeout(() => { upsertDay(date, dayData); }, 1000);
  ...
  const updateKapida = (v) => setDayData(p => ({ ...p, kapidaOdeme: v }));
  const addSarf    = () => setDayData(p => ({ ...p, sarfMalzeme: [...(p.sarfMalzeme||[]), { id: Date.now(), ad: "", tutar: "" }] }));
  const updateSarf = (id, f, v) => setDayData(p => ({ ...p, sarfMalzeme: p.sarfMalzeme.map(x => x.id === id ? { ...x, [f]: v } : x) }));
  const delSarf    = (id) => setDayData(p => ({ ...p, sarfMalzeme: p.sarfMalzeme.filter(x => x.id !== id) }));
  ...
  return (
    <div style={{ ...S.root, background: dark ? "#0f172a" : "#f3f4f6", color: dark ? "#f1f5f9" : "#111827" }}>
      <nav style={{ ...S.nav, background: dark ? "#1e293b" : "#fff", borderColor: dark ? "#334155" : "#e5e7eb" }}>
        <div style={S.navLeft}>
        ...
        <div style={S.navRight}>
          <button onClick={() => setDark(d => !d)} style={S.navBtn} title="Gece Modu">
            {dark ? "☀️" : "🌙"}
          </button>
      ...
      {view === "entry"
        ? <EntryView date={date} setDate={setDate} dayData={dayData} calc={calc} loading={loading}
            ...
            updateStok={updateStok} updateKapida={updateKapida} addSarf={addSarf} updateSarf={updateSarf} delSarf={delSarf} onSave={handleSave} saving={saving} dark={dark} />
        : <DashboardView y={selYear} m={selMonth} setY={setSelYear} setM={setSelMonth} dark={dark} />
      }
    </div>
  );
}
...
// ENTRY VIEW
// ═════════════════════════════════════════════════════════════════════════════
function EntryView({ date, setDate, dayData, calc, loading, updateCiro, updateBorek, addEleman, updateEleman, delEleman, addMasraf, updateMasraf, delMasraf, updateStok, updateKapida, addSarf, updateSarf, delSarf, onSave, saving, dark }) {
  ...
  return (
    <div style={{ ...S.entryWrap, background: dark ? "#0f172a" : "#f3f4f6" }}>
      <div style={S.entryInner}>
        ...
        {(() => {
          const ciroFields = [dayData.ciro.nakit, dayData.ciro.kart, dayData.ciro.yemek, dayData.ciro.trendyol, dayData.ciro.yemeksepeti, dayData.ciro.getir, dayData.ciro.migros];
          const isInvalidVal = (v) => v !== "" && v !== null && v !== undefined && !/^-?\d*\.?\d*$/.test(String(v).trim());
          ...
          const sectionBg = hasInvalid ? (dark ? "#450a0a" : "#fff1f2") : dolu === 0 ? (dark ? "#1e293b" : "#fff") : dolu === toplam ? (dark ? "#14532d" : "#f0fdf4") : (dark ? "#450a0a" : "#fff1f2");
          return (
        <div style={{ ...S.section, background: sectionBg, transition:"background 0.3s" }}>
          <div style={S.sectionHeader}>
            <span style={{ marginRight:8, fontSize:20 }}>💳</span>
            <h2 style={{ ...S.sectionTitle, color:"#2563eb" }}>Ciro (Revenue)</h2>
          </div>
          {(() => {
            const isInvalid = (v) => v !== "" && v !== null && !/^-?\d*\.?\d*$/.test(String(v).trim());
            const iStyle = (v) => ({ ...S.input, ...(isInvalid(v) ? { borderColor:"#ef4444", background:"#fff1f2" } : {}) });
            return (
            <div style={S.grid2}>
              <Field label="KREDİ KARTI">
                <input type="text" placeholder="0.00" value={dayData.ciro.kart} onChange={e => updateCiro("kart", e.target.value)} style={iStyle(dayData.ciro.kart)} />
                {num(dayData.ciro.kart) > 0 && <div style={S.otoHint}>Komisyon: ₺{fmt(oto.kart)}</div>}
              </Field>
              <Field label="NAKİT">
                <input type="text" placeholder="0.00" value={dayData.ciro.nakit} onChange={e => updateCiro("nakit", e.target.value)} style={iStyle(dayData.ciro.nakit)} />
              </Field>
              <Field label="YEMEK KARTI">
                <input type="text" placeholder="0.00" value={dayData.ciro.yemek} onChange={e => updateCiro("yemek", e.target.value)} style={iStyle(dayData.ciro.yemek)} />
            ...
            {(() => {
              ...
              <div style={S.grid2}>
                <Field label="TRENDYOL YEMEK">
                  <input type="text" placeholder="0.00" value={dayData.ciro.trendyol} onChange={e => updateCiro("trendyol", e.target.value)} style={iStyle(dayData.ciro.trendyol)} />
                </Field>
                <Field label="YEMEK SEPETİ">
                  <input type="text" placeholder="0.00" value={dayData.ciro.yemeksepeti} onChange={e => updateCiro("yemeksepeti", e.target.value)} style={iStyle(dayData.ciro.yemeksepeti)} />
                </Field>
                <Field label="GETİR YEMEK">
                  <input type="text" placeholder="0.00" value={dayData.ciro.getir} onChange={e => updateCiro("getir", e.target.value)} style={iStyle(dayData.ciro.getir)} />
                </Field>
                <Field label="MİGROS YEMEK">
                  <input type="text" placeholder="0.00" value={dayData.ciro.migros} onChange={e => updateCiro("migros", e.target.value)} style={iStyle(dayData.ciro.migros)} />
                </Field>
              </div>
              );
            })()}
        ...
        <Section icon="🧾" color="#dc2626" title="Günlük Masraflar" dark={dark}>
          ...
          {/* Paket Yemek Kartı - Kapıda Ödeme */}
          <div style={S.subSection}>
            <div style={S.subHeader}>
              <span style={S.subTitle}>Paket Yemek Kartı - Kapıda Ödeme</span>
            </div>
            <div style={S.masrafRow}>
              <input
                type="number" placeholder="Tutar giriniz"
                value={dayData.kapidaOdeme || ""}
                onChange={e => updateKapida(e.target.value)}
                style={{ ...S.input, flex: 2 }}
              />
              <div style={{ flex:1, padding:"10px 12px", background:"#fff7ed", border:"1px solid #fed7aa", borderRadius:8, fontSize:13, color:"#c2410c", fontWeight:600, textAlign:"center" }}>
                -{fmt(num(dayData.kapidaOdeme) * 0.10)} ₺
              </div>
            </div>
            {num(dayData.kapidaOdeme) > 0 && (
              <div style={{ fontSize:11, color:"#9ca3af", marginTop:4 }}>
                {fmt(num(dayData.kapidaOdeme))} ₺ × %10 = {fmt(num(dayData.kapidaOdeme) * 0.10)} ₺ masraf
              </div>
            )}
          </div>
        ...
        <Section icon="🥐" color="#d97706" title="Börek Maliyeti" dark={dark}>
          <div style={S.grid2}>
            {BOREK_URUNLER.map(u => (
          ...
          <div style={{ marginTop:14, paddingTop:12, borderTop:"1px dashed #e5e7eb" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <span style={{ fontSize:13, fontWeight:600, color:"#374151" }}>Sarf Malzeme</span>
              <button onClick={addSarf} style={S.addBtn}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                Ekle
              </button>
            </div>
            {(dayData.sarfMalzeme||[]).length === 0
              ? <div style={S.emptyRow}>Sarf malzeme eklenmedi</div>
              : (dayData.sarfMalzeme||[]).map(x => (
                <div key={x.id} style={S.masrafRow}>
                  <input placeholder="Malzeme adı" value={x.ad} onChange={e => updateSarf(x.id,"ad",e.target.value)} style={{ ...S.input, flex:2 }} />
                  <input placeholder="₺" type="number" value={x.tutar} onChange={e => updateSarf(x.id,"tutar",e.target.value)} style={{ ...S.input, flex:1 }} />
                  <button onClick={() => delSarf(x.id)} style={S.delBtn}>🗑</button>
                </div>
              ))
            }
          </div>
        ...
        <Section icon="📦" color="#0891b2" title="Stok Yönetimi" dark={dark}>
        ...
        <Section icon="💰" color="#16a34a" title="Net Kar (Net Profit)" dark={dark}>
...
// DASHBOARD VIEW
// ═════════════════════════════════════════════════════════════════════════════
function DashboardView({ y, m, setY, setM, dark }) {
  ...
  const updFatura = async (id, v) => {
    const updated = { ...monthData, faturalar: (monthData.faturalar || defaultFaturalar()).map(x => x.id === id ? { ...x, tutar: v } : x) };
  ...
  return (
    <div style={{ ...SD.landing, background: dark ? "#0f172a" : "#f3f4f6" }}>
      <div style={{ ...SD.leftPanel, background: dark ? "#0f172a" : "#f3f4f6" }}>
        <div style={SD.leftInner}>
          ...
          <div style={SD.metricsGrid}>
            ...
            <MiniMetric label="Net Kar"     value={`₺${fmt(stats.netKar)}`}     color={stats.netKar >= 0 ? "#16a34a":"#dc2626"} bg={stats.netKar >= 0 ? "#f0fdf4":"#fff1f2"} dark={dark} />
            <MiniMetric label="Ort. Günlük" value={`₺${fmt(stats.avgCiro)}`}    color="#7c3aed" bg="#f5f3ff" dark={dark} />
            <MiniMetric label="Aktif Gün"   value={`${stats.activeDays} gün`}   color="#d97706" bg="#fffbeb" dark={dark} />
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <div style={SD.card}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
                ...
                <button onClick={addSabit} style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:"#fff", background:"#2563eb", border:"none", cursor:"pointer", fontWeight:600, padding:"5px 10px", borderRadius:7 }}>
              ...
              <div style={SD.sabitToplam}>
                <span style={{ fontSize:11, fontWeight:700, color:"#6b7280", letterSpacing:"0.05em" }}>TOPLAM SABİT</span>
              ...
              <div style={{ marginTop:14, paddingTop:12, borderTop:"2px dashed #e5e7eb" }}>
                <div style={{ fontSize:12, fontWeight:700, color:"#0891b2", marginBottom:8, letterSpacing:"0.04em" }}>FATURALAR</div>
                {(monthData.faturalar || defaultFaturalar()).map(x => (
                  <div key={x.id} style={SD.sabitRow}>
                    <span style={SD.sabitLabel}>{x.ad}</span>
                    <div style={SD.sabitInputWrap}>
                      <span style={SD.sabitCur}>₺</span>
                      <input type="number" placeholder="0" value={x.tutar} onChange={e => updFatura(x.id, e.target.value)} style={SD.sabitInput} />
                    </div>
                  </div>
                ))}
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:10, paddingTop:8, borderTop:"1px solid #f3f4f6" }}>
                  <span style={{ fontSize:11, fontWeight:700, color:"#6b7280", letterSpacing:"0.05em" }}>TOPLAM FATURA</span>
                  <span style={{ fontSize:15, fontWeight:800, color:"#0891b2" }}>₺{fmt(stats.faturaTotal)}</span>
                </div>
              </div>
            </div>
            ...
            <div style={SD.card}>
              ...
              <div style={{ marginTop:8 }}>
                ...
                <KirilimRow label="Faturalar"     val={`-₺${fmt(stats.faturaTotal)}`} color="#0891b2" />
...
function MiniMetric({ label, value, color, bg, dark }) {
  return (
    <div style={{ background: dark ? "#1e293b" : bg, borderRadius:10, padding:"12px 14px" }}>
...
function Section({ icon, color, title, children, dark }) {
  return (
    <div style={{ ...S.section, background: dark ? "#1e293b" : "#fff", color: dark ? "#f1f5f9" : "#111827" }}>
      <div style={S.sectionHeader}>
...
// ═════════════════════════════════════════════════════════════════════════════
const S = {
  root:    { minHeight:"100vh", background:"var(--bg)", fontFamily:"'Inter', system-ui, sans-serif" },
  nav:           { display:"flex", justifyContent:"space-between", alignItems:"center", padding:"0 24px", height:56, background:"#fff", borderBottom:"1px solid #e5e7eb", position:"sticky", top:0, zIndex:100, transition:"background 0.2s" },
  ...
  entryWrap:     { minHeight:"calc(100vh - 56px)", padding:"32px 16px 60px" },
  ...
  section:       { borderRadius:12, padding:"20px 20px 16px", marginBottom:16, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" },
  ...
  input:    { padding:"10px 12px", border:"1px solid #e5e7eb", borderRadius:8, fontSize:14, color:"#111827", background:"#fff", width:"100%", boxSizing:"border-box", transition:"background 0.2s" },
  ...
  addBtn:     { display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#fff", background:"#2563eb", border:"none", cursor:"pointer", fontWeight:600, padding:"6px 12px", borderRadius:8 },
...
const SD = {
  landing:   { minHeight:"calc(100vh - 56px)" },
  ...
  card:          { borderRadius:12, padding:"16px", marginBottom:12, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" },
